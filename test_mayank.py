...
def test(cfg,
        ...
        for si, pred in enumerate(output):
            ...
            if save_json:
                ...
                for p, b in zip(pred.tolist(), box.tolist()):
                    # print(p)
                    jdict.append({'image_id': image_id, 'category_id': coco91class[int(p[5])], 'bbox': [round(x, 3) for x in b], 'score': round(p[4], 5)})
                    # jdict.append({'image_id': image_id, 'category_id': [int(p[5])], 'bbox': [round(x, 3) for x in b], 'score': round(p[4], 5)})
    ...
    if save_json and map and len(jdict):
        ...
        try:
            ...
            cocoGt = COCO(glob.glob('/home/mayank_s/codebase/cplus_plus/ai/darknet_AlexeyAB/coco/annotations/instances_val*.json')[0])  # initialize COCO ground truth api
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', type=int, default=412, help='inference size (pixels)')
